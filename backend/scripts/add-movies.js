...
try {
  ...
  // Add at least one relation to every movie, if missing, default link to "The Godfather" or some existing
  // Also we want to add the specific similarities.
  ...
  for (const m of NEW_MOVIES) {
      if (!titlesWithPairs.has(m.title) && titlesWithPairs.has("The Godfather")) {
          sims.push([m.title, "The Godfather"]);
      } else if (!titlesWithPairs.has(m.title)) {
          sims.push([m.title, existingTitles.values().next().value]);
      }
  }
  ...
  console.log("Movies added. New sizes:", movies.length, sims.length);
} catch (e) {
